...
export async function POST(request: Request) {
  ...
  try {
    await commitFile(
      `public/data/${month}-respuesta.json`,
    ...
    if (existing) {
      existing.hasRespuesta = true;
    } else {
      const entry: MonthEntry = { key: month, label, hasActividades: false, hasRespuesta: true };
      index.months.push(entry);
      index.months.sort((a, b) => b.key.localeCompare(a.key));
    }
    await commitFile('public/data/index.json', JSON.stringify(index, null, 2), `index: respuesta ${label}`);
